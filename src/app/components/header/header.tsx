    ...
    return(
        <div className={styles.header}>
            ...
            <Popup open={showLogin} setOpen={setShowLogin} exitButton>
                ...
                <motion.button whileTap={{ scale: 0.95 }} transition={{ duration: 0.05 }} className={styles.provider_button} onClick={() => signInWithGoogle().finally(() => setShowLogin(false))}>
                    <Image src='/images/providers/google.png' alt="google" width={25} height={25} />
                    Sign in with Google
                </motion.button>
                <motion.button whileTap={{ scale: 0.95 }} transition={{ duration: 0.05 }} className={styles.provider_button} onClick={() => signInWithMicrosoft().finally(() => setShowLogin(false))}>
                    <Image src='/images/providers/microsoft.png' alt="microsoft" width={25} height={25} />
                    Sign in with Microsoft
                </motion.button>
                <motion.button whileTap={{ scale: 0.95 }} transition={{ duration: 0.05 }} className={styles.provider_button} onClick={() => signInWithGithub().finally(() => setShowLogin(false))}>
